...
LEFT JOIN {{ ref('dim_transaction_types') }} as tt on s.transaction_type_id = tt.transaction_type_id
LEFT JOIN {{ ref('dim_currencies') }} as c on s.currency_id = c.currency_id
LEFT JOIN {{ ref('dim_channels') }} as ch on s.channel_id = ch.channel_id
